...
Http.prototype._request = function (options, callback) {
  ...
  req = (this.ssl ? https : http).request({
    ...
    path: '/' + path.replace(/^\//, ''),
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'X-Token-Authorization': auth },
    agent: this.agent
  });
  ...
  req.end(new Buffer(JSON.stringify({message: options.params.message, additionalInfos: JSON.stringify(options.params.meta), source: this.source, timestamp: this.timestamp}), 'utf8'));
};
